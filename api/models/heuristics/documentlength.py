...
class DocumentLength(Heuristic):
    ...
    def score(eula):
        ...
        feedback = [{'rating': 3, 'text': 'This EULA is {0} words long'.format(words)}]

        if words < 1200:
            score = 4
            grade = 'A'
            feedback.append({'rating': 2, 'text': 'This EULA is short and succinct'})
        elif words < 1700:
            score = 3
            grade = 'B'
            feedback.append({'rating': 1, 'text': 'This EULA is relatively concise'})
        elif words < 2000:
            score = 2
            grade = 'C'
